...
async function signIn(req, res) {
  ...
  if (!result[0] || !body.password || !body.email) {
    ...
    if (codeMatchResult) {
      ...
      res.cookie("authcookie", refresh_token, { httpOnly: true }).send({
        status: 1,
        id: result[0].id,
        message: "logged",
      ...
      res.status(200).send({ status: 0, message: "the password if incorrect" });
    }
  }
}
...
async function updateOne(req, res) {
  ...
  try {
    if (req.status === 0 || req.status === -1) {
      ...
      await model.findOneAndUpdate(req.id, req.id, data[0]);
